...
class estudiante
{
	...
	function consultarTodo($conexion,$est_ced,$fil_codlapso)
	{
		...
		$respuesta['inhabilitadoinsc'] = false; //Status para saber si esta inhabilitado inscripcion por rechazo lista negr
		...
		$sql="select * 
			from eva_rechazoinsc
			where rin_cedalum='$est_ced';";
		//echo $sql;
		$ok=$conexion->ejecutarQuery($sql);
		if($ok)
		{
			...
			if($filas>0)
			{
				...
				//$respuesta['mensaje'] = "Encontrada en rechazados.";
				$datos=mysql_fetch_assoc($ok); //Crea Vector asociativo con el mismo nombre de los datos de la Tabla
				$respuesta['mensaje'] = "Inhabilitado para Inscripción por: " . $datos['rin_descrip'];
				$respuesta['inhabilitadoinsc'] = true;
				$respuesta['tablaest'] = $datos;
				echo json_encode($respuesta);
				return 0;
			}
...
*/
	...
	{	
		...
		$sta_inscviva = 1; //Estatus para saber si la inscripcion finalizo o esta viva
		if($tipousuario == 4) //solo para los estudiantes, los administratios si opueden hacer inscripcion
		{
			$sql = "select *,DATE_FORMAT(fil_fecha_dep,'%d/%m/%Y') as fil_fecha_dep_dma,if(now()<fil_fecfininsc,1,0) as sta_inscviva from eva_filtros;";
			//echo $sql;
			$ok=$conexion->ejecutarQuery($sql);
			//echo $ok;
			if($ok)
			{
				...
				if($filas>0)
				{
					$obfiltros=$conexion->objeto($sql);
					$sta_inscviva = $obfiltros->sta_inscviva;
				}
			}
		}
		/*
		echo $dato->est_nombres;
		echo "  -  ";
		echo $dato->est_ced;
		*/
		if(($dato->est_ced > 0) && ($sta_inscviva == 1))
		{
			//buscar Estudiante
...
/*
		...
		}else
		{
			if($sta_inscviva == 0)
			{
				$respuesta['mensaje'] = "Inscripción Finalizó.";
			}else
			{
